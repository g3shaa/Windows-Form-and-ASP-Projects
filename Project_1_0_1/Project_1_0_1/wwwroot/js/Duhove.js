...
Ghost.prototype.getTestDistance = function(dir, targetPacman) {
	...
	this.moveOneStep();
	if(targetPacman){
		toReturn = Math.sqrt(Math.pow( (this.x - PacYMan.x)  ,2)+Math.pow( this.y -PacYMan.y,2));
	}
	else{
		switch(PacYMan.dir){
			case LEFT:
			toReturn = Math.sqrt(Math.pow( (this.x - (PacYMan.x - 4*GRID_WIDTH))  ,2)+Math.pow( this.y -PacYMan.y,2));
			break;
			...
			case UP:
			toReturn = Math.sqrt(Math.pow( (this.x - PacYMan.x)  ,2)+Math.pow( this.y - (PacYMan.y - 4*GRID_HEIGHT),2));
			break;
