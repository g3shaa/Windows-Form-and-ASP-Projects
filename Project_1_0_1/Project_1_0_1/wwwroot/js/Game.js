...
function printInstruction () {
	...
	if (ghosts.length === 0){
		...
		ctx.textAlign = "left";
		ctx.fillText("DEV MODE", x, CANVAS_WIDTH-20);
	}
...
function startingScreen(){
	...
	ctx.fillText("Press E to start", CANVAS_WIDTH/2, 220);
	ctx.font = "14px bahnschrift";
	ctx.fillText("Press G for DEV MODE", CANVAS_WIDTH/2, 250);
	ctx.fillText("Discord: g3shaa#2689 / Github: g3shaa", CANVAS_WIDTH/2 , CANVAS_HEIGHT/20*19);

	welcomePacman = new Pacman(CANVAS_WIDTH/5, CANVAS_HEIGHT/3*2, RIGHT);
	welcomePacman.radius = 30;
	welcomePacman.draw();

	welcomeGeshata = new Ghost(CANVAS_WIDTH/5*3.3, CANVAS_HEIGHT/3*2, RED, LEFT);
	welcomeGeshata.radius = 30;
	welcomeGeshata.draw();

	welcomePesho = new Ghost(CANVAS_WIDTH/5*4, CANVAS_HEIGHT/3*2, CYAN, RIGHT);
	welcomePesho.radius = 30;
	welcomePesho.draw();
	intervalId = setInterval(updatestartingScreen, timerDelay*2);
}
...
function updatestartingScreen () {
	...
	welcomeGeshata.isMoving = !welcomeGeshata.isMoving;
	welcomePesho.isMoving = !welcomePesho.isMoving;
	welcomePacman.draw();
	welcomePesho.draw();
	welcomeGeshata.draw();
}
...
//try to eat a bean
function eatBean () {
	if(onGridCenter(PacYMan.x, PacYMan.y)){
		if(maze[PacYMan.getRow()][PacYMan.getCol()].beanType===NORMAL_BEAN){
			score+= parseInt(10);
			showScore();
			beansLeft--;
		}
		else if (maze[PacYMan.getRow()][PacYMan.getCol()].beanType===POWER_BEAN){
			score+=parseInt(50);
			showScore();
			beansLeft--;
		...
		maze[PacYMan.getRow()][PacYMan.getCol()].beanType=undefined;
		maze[PacYMan.getRow()][PacYMan.getCol()].draw();
	}
}
...
function eatGhost () {
	for(var i=0; i<ghosts.length; i++){
		if(Math.abs(PacYMan.x-ghosts[i].x)<=5 && Math.abs(PacYMan.y-ghosts[i].y)<=5
...
function onKeyDown (event) {
	...
	var godModeCode = 71; //g to enter DEV MODE
	...
	if(!gameOn){
		...
		switch(keycode){
			case upCode:
			case wCode:
			PacYMan.nextDir = PacYMan.dir===UP ? undefined: UP;
			break;

			case rightCode:
			case dCode:
			PacYMan.nextDir = PacYMan.dir===RIGHT? undefined : RIGHT;
			break;

			case leftCode:
			case aCode:
			PacYMan.nextDir = PacYMan.dir === LEFT? undefined : LEFT;
			break;

			case downCode:
			case sCode:
			PacYMan.nextDir = PacYMan.dir === DOWN? undefined : DOWN;
			break;
...
//run the game. Create PacYMan and 4 ghosts. Reset their positions.
function run(isGodMode) {
	showScore();
    
    PacYMan = new Pacman(pacmanStartLoc[1]*GRID_WIDTH + GRID_WIDTH/2, pacmanStartLoc[0]*GRID_HEIGHT + GRID_HEIGHT/2, RIGHT);
    if(isGodMode===undefined || !isGodMode){
	    Geshata = new Ghost(0,0, RED, DOWN);
	    Pesho = new Ghost(0,0, CYAN, DOWN);
	    Icaka = new Ghost(0,0, PINK, DOWN);
	    Alex = new Ghost(0,0, ORANGE, DOWN);

	    Geshata.toGhostHouse();
	    Pesho.toGhostHouse();
	    Icaka.toGhostHouse();
	    Alex.toGhostHouse();

	    ghosts = [Geshata, Pesho, Icaka, Alex];
